...
- **fail2ban resilience + EPEL hygiene + fail-soft**: EPEL is needed only for fail2ban but, left enabled while its Fedora mirrors are unreachable (common from some regions), it makes **every** dnf op — base packages, Docker — hang refreshing EPEL metadata. So EPEL is now installed **disabled-by-default** and enabled **on-demand** (`enablerepo: epel`) only for the fail2ban install; a pre-RHEL-dnf step neutralizes an EPEL left enabled by a prior partial run (pure config edits, can't hang). Because `enablerepo: epel` fans out to dozens of metalink mirrors (dnf's per-connection `timeout`/`retries` do not bound the total), a `timeout -k 5 30 dnf makecache` probe caps the EPEL wait at 30s and kills the process; the fail2ban install is gated on that probe. On EPEL-unreachable (or any failure via `block/rescue`) the play sets `vps_fail2ban_deferred`, warns, and **continues** — fail2ban is optional hardening, re-addable via `modify --toggle-fail2ban on`. Downstream fail2ban tasks (jail/service/verify) skip when deferred.
- **firewalld support (RedHat family)**: mirrors the Debian/UFW rule set with `ansible.posix.firewalld`, but only when firewalld is the host's *active* firewall (detected via `service_facts`); skips cleanly when firewalld is absent/stopped (host uses nftables / cloud SG). Opens the managed SSH port + bootstrap port + requested TCP ports + http/https, ensures firewalld enabled at boot, and removes the bootstrap port after final validation. firewalld is deny-incoming-by-default, so no explicit default-policy task is needed.
- **SELinux managed-SSH-port label**: under SELinux enforcing, sshd may only bind `ssh_port_t`-labeled ports (22 by default), so migrating to the managed port (e.g. 1156) silently failed to listen. `community.general.seport` now labels the managed port `ssh_port_t/tcp` before the transitional sshd reload (RedHat + SELinux enabled; harmless under permissive, skipped when disabled). `policycoreutils-python-utils` installed first for the `semanage` bindings.

### VPS Runner: first-time onboard clears stale host keys (feat/vps-manager-v2, 2026-05-24)

- **Host-key change handling**: first-time onboard now purges stale `~/.ssh/known_hosts` entries for the target before the bootstrap SSH connection. IP reuse / OS reinstall changes a VPS's host key; `ansible.cfg`'s `StrictHostKeyChecking=no` auto-accepts *unknown* hosts but still hard-fails — and disables password auth — on a *changed* key (`REMOTE HOST IDENTIFICATION HAS CHANGED!`), which previously aborted onboard entirely. New `core.clear_known_host(host, *ports)` runs `ssh-keygen -R` for the bare host plus `[host]:bootstrap_port` / `[host]:managed_port` (backup → `known_hosts.old`), so the connection re-pins the new key (TOFU preserved). Wired into all three first-time paths: CLI `onboard --first-time` bootstrap branch + wizard password/key branches.
- **fail2ban on RHEL family**: `onboard.yml` now enables EPEL (`epel-release`, RedHat-gated) before installing fail2ban — fail2ban is not in RHEL/AlmaLinux/Rocky base repos, so onboard previously failed with `No package fail2ban available`. The `python3-systemd` dependency (needed by the jail's `backend = systemd`) is now installed whenever `service_mgr == systemd`, not only on Debian, so the RHEL systemd backend works too.
- **Wizard IP-field resolvability check**: `add-host` wizard now rejects a charset-valid but unresolvable IP/hostname at the prompt (new `core.host_resolves`) instead of letting it through to fail ~3 tasks into the Ansible run as `Could not resolve hostname`. Catches the common typo of entering the alias in the IP field; IP literals always pass, and an explicit `y` override is offered for hosts not yet in DNS.
