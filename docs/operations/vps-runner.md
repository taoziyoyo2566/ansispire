...
| `REMOTE HOST IDENTIFICATION HAS CHANGED!` / `Host key verification failed` | IP 复用或系统重装，远端 host key 变了；本地 `known_hosts` 还存着旧 key。`StrictHostKeyChecking=no` 只接受**未知**主机，对**变更**的 key 仍硬失败并禁用密码认证 | first-time onboard 现已自动处理：连接前对目标 host 及 `[host]:bootstrap_port` / `[host]:managed_port` 跑 `ssh-keygen -R` 清旧条目并重新 pin 新 key（备份到 `known_hosts.old`）。仍报错时手工 `ssh-keygen -R <host>` 后重跑 |
