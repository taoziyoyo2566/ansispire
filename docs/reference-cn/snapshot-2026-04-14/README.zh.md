...
本项目的实际定位是**基于 Ansible 的多服务器管理控制系统**，分"数据面"（roles/playbooks）与"控制面"（controller/）两层。

---

## 控制面（Control Plane）

`controller/` 目录提供一个**轻量开源控制面**，让 Ansible 从"脚本 + SSH"升级为"有 API、有 Job 历史、有凭证管理"的系统。

当前实现：**Semaphore + Docker Compose + SQLite**（约 200 MB RAM）。

```bash
# 快速启动
cp controller/semaphore/.env.example controller/semaphore/.env
vim controller/semaphore/.env        # 至少修改 SEMAPHORE_ADMIN_PASSWORD
make controller-up                    # docker compose up -d
# 浏览器打开 http://localhost:3000
```

选型理由、升级路径、与 AWX/AAP 对比见 [`controller/README.md`](./controller/README.md)。
