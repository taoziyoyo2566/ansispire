...
- Submit a draft to pending: `make vps-submit FILE=<draft.yml>`
...
- A repeated `onboard` for an active alias is rejected; changes must use
  `modify` and reinstall semantics should become a separate future action.
...
- `reinstall`, `rotate_key`, Cloudflare Tunnel provisioning, and signed task
  files are future actions, not part of the MVP.
