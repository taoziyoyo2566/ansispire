...
| ~~`TSVS-VPS-MANAGER-UNIT-001`~~ | 2026-05-14 | ~~`plugins/vps_manager/`（本地 task lifecycle）~~ | L1 | — | **Retired 2026-05-16**（plugin removed, Round 4-b cutover） | — |
| `TSVS-VPS-RUNNER-UNIT-001` | 2026-05-16 | `plugins/vps_runner/`（CLI + core 单元 + ansible-runner integration） | L1+L2 | `make test-vps-runner` + `make test-vps-runner-integration` | Active | [feature-map §Tests](../feature-map/vps-runner.md#tests) |
...
| `plugins/vps_runner/` | `TSVS-VPS-RUNNER-UNIT-001` | ⚠ L1 unit + L2 ansible-runner integration（TEST-NET-1）；3 节点实机并发待 dev VPS SSH 恢复 |
...
| 2026-05-16 | Retired `TSVS-VPS-MANAGER-UNIT-001`（plugin removed）+ 新增 `TSVS-VPS-RUNNER-UNIT-001`（ansible-runner native，21 unit + 1 integration） | feat-vps-manager-v2 round 4-b cutover |
