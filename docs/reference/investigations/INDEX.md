...
| IVG-TOOLENV-REGISTRY | 2026-04-27 / impl 2026-06-10 / hoisted 2026-07-11 | AI-Governance | 架构探索 | 命令执行注册表（TER）：将 AI 工具调用经验外化为可持久化查找表，消除跨会话试错；per-host + timestamp 形态，2026-07-11 上移至 workspace 层（`~/workspace/scripts/env_probe.sh` → `~/workspace/.agents/env/<host>.yml`，workspace-meta 共享），ansispire 仅留项目 override | Applied | `~/workspace/.agents/rules/environment-truth.md`（项目 override：[.agents/env/README.md](../../../.agents/env/README.md)） | [Link](./IVG-TOOLENV-REGISTRY.md) |
