...
- `plan-*.md`: active intent, boundaries, decisions, gates.
- `design-*.md`: stable target-state description or RFC-style summary.
- `round*.changelog.md`: what actually landed in that round.
- `_archive/`: immutable history unless the task is explicitly archival hygiene.
