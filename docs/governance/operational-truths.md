...
- **Internal control-plane traffic is plaintext, by design**: audit-relay / sink / reactor reach Semaphore over the shared `controller-net` docker network (service alias `semaphore`, see `controller/semaphore/docker-compose.yml`). This traffic never leaves the host, so no internal TLS / mTLS is added (decision 2026-06-06, `feat/target-architecture` Q1). External access is terminated at the Cloudflare edge via CF Tunnel — there is no inbound host port. **Precondition**: the audit stack and Semaphore run on the same host / same docker network. **Re-evaluation trigger**: if they are ever split across hosts (cross-network traffic), internal mTLS must be reconsidered before that split ships.
