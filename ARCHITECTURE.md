...
- **Plugin Plane** (`plugins/`): Lightweight local extensions. `vps_runner` is the ansible-runner-native VPS lifecycle plugin — standard `inventory/vps_runner/<env>/` + CLI `list|audit|onboard|modify|remove`. The legacy `vps_manager` (inbox-driven, custom callback, custom state) is retained alongside until the Round 4-b cutover; new work uses `vps_runner`.
...
- [VPS Runner Plugin](docs/reference/feature-map/vps-runner.md) — current (ansible-runner native)
- [VPS Manager Plugin](docs/reference/feature-map/vps-manager.md) — legacy, slated for Round 4-b cutover
