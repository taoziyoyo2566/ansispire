...
- **下一步（按 Jun-10 approved plan 执行，不另开新仓）**：
    ...
    3. `[ ]` Phase 2：修 `onboard.yml` 容器凭据契约（`public_key_content` 分支 + managed 私钥只读 volume mount），同步 examples，新增 `VPS Onboard` 模板和 preflight 断言。**注意复用 Phase 1 的 Environment-`env` + template-binding 模式,避免重犯 vault 中止。**
