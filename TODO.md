...
- **下一步（按 Jun-10 approved plan 执行，不另开新仓）**：
    ...
    3. `[✓]` Phase 2（2026-07-11，代码就绪；live 无真机运行——那是 Phase 3）：`onboard.yml` 加 `public_key_content` 内联分支(显式 if/else 避免 eager lookup)；docker-compose 加 `./secrets:/etc/ansispire/keys:ro` 只读挂载 + `secrets/{.gitignore,README}` + 根 .gitignore；examples 改用 `public_key_content` + 容器私钥路径；`bootstrap.yml` 新增 `vps-onboard-env` + `VPS Onboard` 模板(复用 Phase 1 env-绑定,live id=15/env=5 已绑)。P2.5 门:syntax/lint/controller-bootstrap(Phase 1+2 断言过)/test-api-contract 全 PASS,diff 无 secret。见 `round11-2026-07-11.changelog.md`。**⚠ 待提交。** 另:`make detect-secrets` 有既存 baseline 漂移(4 处非本轮文件),独立 secret-hygiene 处理。
