...
- **下一步（按 Jun-10 approved plan 执行，不另开新仓）**：
    ...
    4. `[~]` Phase 3（2026-07-11，2/3 真机证明）：**u24 + d13 完整闭环——onboard → 切 39222 managed 通道、关 22 → managed 重审 `success` + `changed=0`**。**r9(Rocky)受阻**：`dnf install fail2ban` 连不上 EPEL 镜像 hung(那台 VPS 基础设施问题,非代码)。剩余：r9 修 EPEL 可达性后重跑、`controller-vps-smoke`、TSVS。**建议 onboard 加 `strategy: free` 避免单台慢机堵全队。**
